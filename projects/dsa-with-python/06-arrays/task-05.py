...
while True:
    ...
    if student_id not in entries:
        new_entry = [student_id, [grade]]
        entries += new_entry
    else:
        for entries_i in range(len(entries)):
            if entries[entries_i] == student_id:
                old_grades = entries[entries_i + 1]
                old_grades += [grade]
        
# sort students grades
# 2 as step cause every second entry is students grades array
for i in range(1, len(entries), 2):
    ...
    is_all_passed = False
    while not is_all_passed:
        for k in range(len(grades) - 1):
            if grades[k] <= grades[k + 1]:
                is_all_passed = True
            else:
                is_all_passed = False
                grades[k], grades[k+1] = grades[k+1], grades[k]
